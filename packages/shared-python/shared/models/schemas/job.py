...
class ParsingParams(BaseModel):
    ...
    kb_dir: Optional[str] = Field("Default_Root", description="Knowledge-base directory")
    doc_type: Literal["auto", "pdf", "docx", "txt", "md"] = Field("auto", description="Document type")
    ...
    add_frag_desc: Optional[str] = Field("", description="Additional fragment description")
...
class JobCreate(BaseModel):
    ...
    file_name: Optional[str] = Field(
        None, description="File name; required when source_type=file and must include the extension"
    )
    data_id: Optional[str] = Field(None, max_length=128, description="User-defined ID")
    parsing_params: Optional[ParsingParams] = Field(None, description="Parsing parameters")
...
class JobResponse(BaseModel):
    ...
    upload_headers: Optional[Dict[str, str]] = Field(None, description="Upload request headers")
