...
class RetrievalExecutionPlan:
    ...
    async def execute(self) -> dict[str, Any]:
        ...
        exclude_document_ids = self.request["exclude_document_ids"]
        exclude_sections = self.request["exclude_sections"]
        data_type = self.request["data_type"]
        signal_paths = self.request["signal_paths"]
        filter_mode = self.request["filter_mode"]
        channels = self.request["channels"]
        channel_weights = self.request["channel_weights"]
        rerank = self.request["rerank"]
        threshold = self.request["threshold"]
        internal_recall_k = self.request["internal_recall_k"]
        use_agentic = self.request["use_agentic"]
