...
from shared.services.retrieval.channels import (
    content_channel,
    path_channel,
    term_channel,
)
from shared.services.retrieval.graph_service import (
    GraphQueryService,
    is_excluded_section,
)
...
async def list_lexical_chunks(
    ...
    *,
    ...
    like = f"%{query}%"
    content_stmt = (
        base_stmt.where(DocumentChunk.content_lexical_text.ilike(like))
        .order_by(DocumentChunk.sort_order)
        .limit(recall_k)
    )
    path_stmt = (
        base_stmt.where(DocumentChunk.path_lexical_text.ilike(like))
        .order_by(DocumentChunk.sort_order)
        .limit(recall_k)
    )
...
def _grep_search_rows(rows: list[dict[str, Any]], query: str) -> list[dict[str, Any]]:
    ...
    for row in rows:
        haystack = (
            str(row.get("content") or "") + " " + str(row.get("section_path") or "")
        ).lower()
...
def schedule_retrieval_hit_stats_update(
    *, user_id: str, namespace: str, results: list[dict[str, Any]]
) -> None:
    try:
        asyncio.create_task(
...
async def _record_retrieval_hit_stats_best_effort(
    *, user_id: str, namespace: str, results: list[dict[str, Any]]
) -> None:
    try:
        async with get_db_context() as db:
            await record_retrieval_hits(
                db, user_id=user_id, namespace=namespace, results=results
            )
            await db.commit()
    except Exception as e:
...
async def generate_retrieval_asset_url(*, job_id: str, artifact_ref: str) -> str | None:
    return get_result_storage().generate_artifact_url(
        job_id=job_id, artifact_ref=artifact_ref
    )
...
async def _to_public_response(response: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in response.get("results", []):
        ...
        if (
            _is_media_chunk(row)
            and _is_client_result_artifact_ref(artifact_ref)
            and row.get("job_id")
        ):
            try:
...
async def _count_scoped_chunks(
    ...
    *,
    ...
    if allowed_chunk_types is not None:
        stmt = stmt.where(
            func.lower(DocumentChunk.chunk_type).in_(list(allowed_chunk_types))
        )
    result = await db.execute(stmt)
...
async def _hydrate_paths_to_rows(
    ...
    *,
    ...
    for document, chunk, section, job_result in result.all():
        row_path = (
            (section.section_path if section else None) or chunk.source_chunk_path or ""
        )
...
async def run_retrieval_query(
    *,
    ...
    effective_recall_k = (
        ...
        else top_k * _INTERNAL_RECALL_K_MULTIPLIER
    )
    ...
    try:
        ...
        if cached:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to trigger retrieval hit stats update (ignored): {e}"
                )
    ...
    if total_chunk_count is not None and total_chunk_count <= top_k:
        logger.info(
            f"retrieval: small_kb_optimization total_chunks={total_chunk_count} <= top_k={top_k}"
        )
        ...
        try:
            schedule_retrieval_hit_stats_update(
                user_id=user_id, namespace=namespace, results=results
            )
        except Exception as e:
            logger.warning(
                f"Failed to trigger retrieval hit stats update (ignored): {e}"
        ...
        logger.info(
            f"retrieval: small_kb response={len(results)} results, total_time={elapsed_total}ms"
        )
    ...
    if "path" in active_channels:
        ...
        logger.info(
            f"retrieval: path_channel={len(path_rows)} rows in {round((time.monotonic() - t_ch) * 1000)}ms"
        )
    ...
    if "content" in active_channels:
        ...
        logger.info(
            f"retrieval: content_channel={len(content_rows)} rows in {round((time.monotonic() - t_ch) * 1000)}ms"
        )
    ...
    if "term" in active_channels:
        ...
        logger.info(
            f"retrieval: term_channel={len(term_rows)} rows in {round((time.monotonic() - t_ch) * 1000)}ms"
        )
    ...
    fused_rows = (
        merge_channels_rrf(channel_lists, weight_list, top_k) if channel_lists else []
    )
    logger.info(
        f"retrieval: rrf_fusion={len(fused_rows)} rows from {len(channel_lists)} channels"
    )
    ...
    if threshold > 0.0 and fused_rows:
        ...
        logger.info(
            f"retrieval: threshold_filter={pre_count}->{len(fused_rows)} (threshold={threshold})"
        )
    ...
    if llm_fn is not None:
        ...
        try:
            ...
            logger.error(
                f"retrieval: agent_navigate failed, falling back to lexical: {exc}"
            )
            agent_rows = await list_graph_routed_chunks(
                db,
                user_id=user_id,
