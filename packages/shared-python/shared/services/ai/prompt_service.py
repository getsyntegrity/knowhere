...
def build_prompt(task, texts, query, **kwargs):
    ...
    """
        ...
        top_p = 0.01
        max_depth = kwargs['paras']['max_depth']
        # min_depth = kwargs['paras']['min_depth']
        max_tokens = kwargs['paras']['max_tokens']
        # basic_preds = kwargs['paras']['basic_preds']

        prompt = f"""
        你是一位文档结构校正专家，你将接收由多行文本组成的html表格，每行文本可能是标题或正文，包括：
        1. id列：文本行号
        2. heading列：文本内容
        3. level列：文本内容初步估计的层级（可能不准确），具体如下
            1 表示 `<h1>`，2 表示 `<h2>`，依此类推
            -1 表示该行文本被估计为“正文”，而不是标题
            Not Sure 表示该行文本层级待定
        ...
        ***你的任务是***【调整文本层级，形成具备明确、准确父子关系的提纲】，你可进行的行动如下：
        1. 如你认为当前行的层级估计准确，保持原状
        2. 如你认为当前行的层级估计不准，将其调整为准确的层级（即1-{max_depth}的整数）
        3. 如你认为当前行更适合作为正文而非标题，将其层级调整为-1

        ***进行以上判断时***要遵守以下原则：
        1. 注意连续候选文本之间的父子层级关系
        2. 连续标题之间的层级不能跳跃（如从1级跳到3级）

        ***输出要求***
        - 输出结果只能为【JSON数组】，数组每个元素应按顺序包含以下字段：
            - "id": 原始行序号（整数）；
            - "level": 该行调整之后的新层级（1~{max_depth} 的整数或 -1）
        ...
        ***其他要求***
        ...
        prompt = f"""
        你是一个严格且客观的冲突检测器。  
        请比较以下两个知识片段的内容，判断它们是否存在事实或逻辑上的冲突。  

        【判断标准】  
        请在以下维度上进行判断：  
        1. **事实冲突**：两段描述对同一事件、现象、定义、结论的表述相反或互斥
        2. **数值冲突**：同一指标、数据、时间、数量、范围等数值不一致
        3. **因果冲突**：A 认为 X 导致 Y，而 B 认为 X 不会或不能导致 Y
        4. **条件冲突**：A 与 B 在适用前提、范围或限定条件上互相矛盾
        5. **语义冲突**：词义或句意导致结论无法同时为真（即便非数值型）
        ...
        【输出要求】  
        仅返回一个合法 JSON 字典，格式如下（字段必须完整）：  
        {{
          "contradict": true/false,        // 是否存在冲突
          "reason": "简要说明冲突或不冲突的原因",
          "conflict_sentences": {{
            "片段A": "若存在冲突，请指出片段A中的相关句子；若无冲突则为空字符串",
            "片段B": "若存在冲突，请指出片段B中的相关句子；若无冲突则为空字符串"
          }}
        }}
