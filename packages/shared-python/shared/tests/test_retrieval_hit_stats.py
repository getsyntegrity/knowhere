...
def test_record_retrieval_hits_deduplicates_document_hits():
    ...
    document_params = [params for params in db.params if params['hit_kind'] == 'document']
...
def test_record_retrieval_hits_uses_atomic_upsert_for_each_logical_hit():
    ...
    assert all('hit_count = retrieval_hit_stats.hit_count + 1' in statement for statement in db.statements)
...
def test_record_retrieval_hits_handles_null_chunk_document_hits_with_partial_conflict_target():
    ...
    assert 'ON CONFLICT (user_id, namespace, hit_kind, document_id)' in document_statement
    assert 'WHERE chunk_id IS NULL' in document_statement
    assert 'ON CONFLICT (user_id, namespace, hit_kind, document_id, chunk_id)' in chunk_statement
