...
def test_publish_document_state_creates_default_namespace_document(monkeypatch) -> None:
    ...
    assert any(
        isinstance(obj, Document) and obj.namespace == "default" for obj in added
    )
    assert any(
        isinstance(obj, DocumentSection) and obj.section_path == "Billing / Refunds"
        for obj in added
    )
    matching_chunks = [
        obj
        ...
        and obj.content == "Refunds within 30 days are allowed."
    ]
...
def test_publish_document_state_uses_file_path_as_internal_media_reference(
    monkeypatch,
) -> None:
    db = MagicMock()
    service = RetrievalPublicationService()
    ...
    matching_chunks = [
        obj
        for obj in added
        if isinstance(obj, DocumentChunk) and obj.chunk_id == "image_1"
    ]
    assert len(matching_chunks) == 1
...
def test_publish_document_state_preserves_existing_document_namespace_when_update_omits_namespace() -> (
    None
):
    db = MagicMock()
    service = RetrievalPublicationService()
    ...
    db.execute.side_effect = [
        ...
        SimpleNamespace(
            scalar_one_or_none=lambda: SimpleNamespace(
                id="result_old", document_id="doc_123"
            )
        ),
        SimpleNamespace(
            scalar_one_or_none=lambda: SimpleNamespace(
                id="result_123", document_id="doc_123"
            )
        ),
...
def test_publish_document_state_preserves_historical_canonical_rows() -> None:
    ...
    delete_calls = [
        call for call in db.execute.call_args_list if "DELETE" in str(call.args[0])
    ]
...
def test_publish_document_state_rejects_stale_completion_for_existing_document() -> (
    None
):
    db = MagicMock()
    service = RetrievalPublicationService()
...
def test_finalize_job_success_invalidates_cache_only_after_commit(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        service,
        "_build_retrieval_cache_invalidation",
        lambda *_args, **_kwargs: {
            "user_id": "user_123",
            "namespaces": ["default"],
            "job_id": "job_123",
    ...
    monkeypatch.setattr(
        service,
        "_post_commit_invalidate_retrieval_cache",
        lambda payload: events.append(("invalidate", payload)),
        raising=False,
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: events.append(("webhook", None)),
    )
...
def test_post_commit_invalidate_retrieval_cache_uses_prefixed_sync_redis_keys(
    monkeypatch,
) -> None:
    service = lifecycle_module.SyncJobLifecycleService()
...
def test_job_lifecycle_sync_keeps_retrieval_publication_logic_out_of_module() -> None:
    source = (Path(__file__).parents[1] / "services/job_lifecycle_sync.py").read_text(
        encoding="utf-8"
    )
