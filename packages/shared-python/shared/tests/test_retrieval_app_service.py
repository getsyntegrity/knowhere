...
@pytest.mark.asyncio
async def test_run_retrieval_query_does_not_overlap_lexical_and_graph_db_work(
    monkeypatch,
):
    from shared.services.retrieval import app_service
    ...
    async def fake_channel(*_args, **_kwargs):
        ...
        tracker["max_active_calls"] = max(
            tracker["max_active_calls"], tracker["active_calls"]
        )
        await asyncio.sleep(0)
        tracker["active_calls"] -= 1
        return []
    ...
    monkeypatch.setattr(
        ...
        fake_get_cached_retrieval_query_result,
    )
    monkeypatch.setattr(
        app_service,
        "set_cached_retrieval_query_result",
        fake_set_cached_retrieval_query_result,
    )
    ...
    monkeypatch.setattr(
        app_service, "list_graph_routed_chunks", fake_list_graph_routed_chunks
    )
    monkeypatch.setattr(
        app_service, "assemble_retrieval_results", fake_assemble_retrieval_results
    )
    monkeypatch.setattr(
        app_service, "schedule_retrieval_hit_stats_update", lambda **_kwargs: None
    )
...
async def test_run_retrieval_query_always_uses_graph_routing(monkeypatch):
    ...
    monkeypatch.setattr(
        app_service, "get_cached_retrieval_query_result", fake_get_cached
    )
    monkeypatch.setattr(
        app_service, "set_cached_retrieval_query_result", fake_set_cached
    )
...
@pytest.mark.asyncio
async def test_run_retrieval_query_serves_cached_result_without_hitting_db_path(
    monkeypatch,
):
    from shared.services.retrieval import app_service
...
async def test_run_retrieval_query_falls_back_to_lexical_when_graph_fails(monkeypatch):
    ...
    assert (
        result["results"][0]["source"]["section_path"] == "Policies / Billing / Refunds"
    )
...
async def test_run_retrieval_query_uses_plan_channel_weights_for_graph_content_path_and_term(
    ...
    monkeypatch.setattr(
        app_service, "set_cached_retrieval_query_result", lambda **_kwargs: None
    )
...
@pytest.mark.asyncio
async def test_run_retrieval_query_returns_asset_url_without_caching_signed_url(
    monkeypatch,
):
    from shared.services.retrieval import app_service
    ...
    public_result = result["results"][0]
    assert (
        public_result["asset_url"]
        == "https://assets.test/job_123/images/page-1.png?signature=fresh"
    )
...
@pytest.mark.asyncio
async def test_run_retrieval_query_adds_fresh_asset_url_to_cached_media_result(
    monkeypatch,
):
    from shared.services.retrieval import app_service
...
@pytest.mark.asyncio
async def test_run_retrieval_query_real_helper_generates_asset_url_in_api_runtime(
    monkeypatch,
):
    from shared.services.retrieval import app_service
...
@pytest.mark.asyncio
async def test_run_retrieval_query_does_not_expose_raw_file_path_when_asset_url_is_unavailable(
    monkeypatch,
):
    from shared.services.retrieval import app_service
...
@pytest.mark.asyncio
async def test_run_retrieval_query_does_not_generate_asset_url_for_text_chunk(
    monkeypatch,
):
    from shared.services.retrieval import app_service
...
@pytest.mark.asyncio
async def test_run_retrieval_query_passes_section_exclusions_to_cache_and_db_paths(
    monkeypatch,
):
    from shared.services.retrieval import app_service
    ...
    exclude_sections = [
        {"document_id": "doc_123", "section_path": "Policies / Billing"}
    ]
...
async def test_assemble_result_content_inlines_connected_tables_after_exclusions():
    ...
    assembled = await app_service.assemble_retrieval_results(
        ...
        exclude_document_ids=[],
        exclude_sections=[
            {"document_id": "doc_123", "section_path": "Policies / Internal Only"}
        ],
    )
...
@pytest.mark.asyncio
async def test_assemble_result_content_hydrates_missing_connected_table_from_canonical_state(
    monkeypatch,
):
    from shared.services.retrieval import app_service
