...
@pytest.mark.asyncio
async def test_bump_namespace_version_increments_and_read_uses_latest_value(
    monkeypatch,
):
    ...
    monkeypatch.setattr(
        cache_service,
        "RedisServiceFactory",
        type("Factory", (), {"get_service": staticmethod(lambda: fake_redis)}),
    )

    version_1 = await cache_service.bump_retrieval_namespace_cache_version(
        user_id="user_123", namespace="default"
    )
    version_2 = await cache_service.bump_retrieval_namespace_cache_version(
        user_id="user_123", namespace="default"
    )
    latest = await cache_service.get_retrieval_namespace_cache_version(
        user_id="user_123", namespace="default"
    )
