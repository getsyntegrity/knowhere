...
class AIConfig(BaseModel):
    ...
    GLM_URL: str = Field(default="https://open.bigmodel.cn/api/paas/v4", description="Zhipu GLM API URL")
    ...
    NORMOL_MODEL: str = Field(default="deepseek-chat", description="Default text model for summaries and general text LLM calls")
    HIERARCHY_LLM_MODEL: str = Field(
        default="",
        description="Heading and outline recognition model; falls back to NORMOL_MODEL when empty",
    )
    IMAGE_MODEL: str = Field(default="qwen3.5-flash", description="Image model for image summary, atlas, and OCR flows")

    # Shared model tuning defaults.
    MIN_CONFIDENCE_THRESHOLD: float = Field(default=0.05, description="Minimum confidence threshold")
    HIGH_IOU_THRESHOLD: float = Field(default=0.9, description="High-IoU threshold")
    DEFAULT_EMBEDDING_DIM: int = Field(default=1024, description="Default embedding dimension")
    ...
    ARK_API_KEY: str = Field(default="", description="ARK API key (compatibility field)")
    ...
    ALI_URL: str = Field(default="https://dashscope.aliyuncs.com/compatible-mode/v1", description="Aliyun DashScope URL (compatibility field)")
    ...
    IMAGE_MODEL_MAX: str = Field(default="qwen3.5-flash", description="Higher-capability image model for OCR and ask-image Q&A")
    REASON_MODEL: str = Field(default="", description="Reasoning model (compatibility field)")
    IMG_HEADER: str = Field(default="", description="Image header (compatibility field)")
    CONFIG_PATH: str = Field(default="app/core/config/config.txt", description="Config path (compatibility field)")
    META_PATH: str = Field(default="app/core/config/Meta_setting.csv", description="Metadata path (compatibility field)")
    IMG_TBL_PATTERN: str = Field(default="", description="Image table pattern (compatibility field)")
    PATH_IMAGE_PATTERN: str = Field(default="", description="Path image pattern (compatibility field)")
    SPLIT_CHAR: str = Field(default="/", description="Path separator")
    ILOVEAPI_PUBLIC_KEY: str = Field(default="", description="iLoveAPI public key (PPTX-to-PDF)")
    ILOVEAPI_SECRET_KEY: str = Field(default="", description="iLoveAPI secret key (PPTX-to-PDF)")
    ILOVEAPI_BASE_URL: str = Field(default="https://api.ilovepdf.com/v1", description="iLoveAPI base URL")
    ILOVEAPI_TIMEOUT: int = Field(default=120, description="iLoveAPI request timeout in seconds")
    ...
    ILOVEAPI_TOKEN_DAILY_LIMIT: int = Field(default=250, description="Per-project daily file limit for iLoveAPI. The free tier is about 250 files per month for officepdf conversions.")
    ...
    PROD_URL: str = Field(default="", description="Production URL (compatibility field)")
    ALL_DF_COLS: str = Field(default="content,path,type,length,keywords,summary,know_id,tokens,connectto,addtime,page_nums", description="All dataframe columns (compatibility field)")
    DEFAULT_FOLDERS: str = Field(default="Supplementary_Files,Temporary_Files,templates,images,fragments", description="Default folders (compatibility field)")
    KB_TERM: str = Field(default="KB_DATA", description="Knowledge-base term (compatibility field)")
    KB_VEC_TERM: str = Field(default="KB_VECS", description="Knowledge-base vector term (compatibility field)")
