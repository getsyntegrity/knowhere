...
class BaseConfig(BaseSettings):
    ...
    APP_ENV: str = Field(default="", description="Deploy environment (<empty>|development|staging|production)")
    DEBUG: bool = Field(default=False, description="Debug mode")
    LOG_LEVEL: str = Field(default="INFO", description="Log level")

    # Application metadata.
    APP_TITLE: str = Field(default="Knowhere API", description="Application title")
    APP_VERSION: str = Field(default_factory=lambda: os.getenv("APP_VERSION", "1.0.0"), description="Application version read from the APP_VERSION environment variable")
    APP_DESCRIPTION: str = Field(default="AI-powered document parsing, retrieval, and knowledge access backend", description="Application description")
    ...
    ACCESS_TOKEN_EXPIRE_MINUTES: int = Field(default=10080, description="Access-token expiration in minutes")
