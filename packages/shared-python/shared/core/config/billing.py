...
class BillingConfig(BaseSettings):
    ...
    FREE_PLAN_CREDITS: int = Field(
        default=100, description="Monthly credits for the free plan"
    )
    PLUS_PLAN_CREDITS: int = Field(
        default=1000, description="Monthly credits for the Plus plan"
    )
    PRO_PLAN_CREDITS: int = Field(
        default=10000, description="Monthly credits for the Pro plan"
    )
    ...
    LOW_BALANCE_THRESHOLD: int = Field(
        default=10_000_000, description="low micro dollars threshold, 10 credits"
    )
    ...
    RESEND_FROM_EMAIL: str = Field(
        default="noreply@knowhere.ai", description="Sender email address"
    )
    RESEND_FROM_NAME: str = Field(
        default="Knowhere AI", description="Sender display name"
    )
    ...
    RESEND_TEMPLATE_WELCOME: Optional[str] = Field(
        default=None, description="Welcome email template ID"
    )
    RESEND_TEMPLATE_PURCHASE_CONFIRMATION: Optional[str] = Field(
        default=None, description="Purchase-confirmation email template ID"
    )
    RESEND_TEMPLATE_JOB_COMPLETION: Optional[str] = Field(
        default=None, description="Job-completion email template ID"
    )
    RESEND_TEMPLATE_JOB_FAILURE: Optional[str] = Field(
        default=None, description="Job-failure email template ID"
    )
    # Resend template feature toggles.
    RESEND_TEMPLATE_WELCOME_ENABLED: bool = Field(
        default=False, description="Enable the welcome email template"
    )
    RESEND_TEMPLATE_PURCHASE_CONFIRMATION_ENABLED: bool = Field(
        default=False, description="Enable the purchase-confirmation email template"
    )
    RESEND_TEMPLATE_JOB_COMPLETION_ENABLED: bool = Field(
        default=False, description="Enable the job-completion email template"
    )
    RESEND_TEMPLATE_JOB_FAILURE_ENABLED: bool = Field(
        default=False, description="Enable the job-failure email template"
    )
