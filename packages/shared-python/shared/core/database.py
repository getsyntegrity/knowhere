...
@asynccontextmanager
async def get_task_local_db_context():
    """
    Create a task-local database context with a fresh engine.
    
    This is designed for Celery tasks that create new event loops.
    The global engine's connection pool is bound to the first event loop,
    which causes 'Event loop is closed' errors when reused across different loops.
    
    This function creates a completely new engine and session for each call,
    ensuring no connection pool state is shared across event loops.
    ...
    """
    # Create a fresh engine for this task (no connection pooling issues)
    task_engine = create_async_engine(
        settings.DATABASE_URL,
        # Use NullPool to avoid connection pooling - each connection is fresh
        poolclass=__import__('sqlalchemy.pool', fromlist=['NullPool']).NullPool,
        connect_args={
            "server_settings": {
                "application_name": "knowhere_worker_task",
                "timezone": "UTC",
    ...
    TaskSessionFactory = sessionmaker(
        bind=task_engine,
        class_=AsyncSession,
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Task-local session rollback failed: {rollback_error}")
            raise commit_error
    except Exception:
        if session:
            try:
                ...
                logger.warning(f"Task-local session rollback failed during exception: {rollback_error}")
        raise
    finally:
        # Close session
        if session:
            try:
                ...
                logger.warning(f"Task-local session close failed: {close_error}")
        
        # Dispose engine to release all connections
        try:
            await task_engine.dispose()
        except Exception as dispose_error:
            logger.warning(f"Task-local engine dispose failed: {dispose_error}")
