...
class KBJobCreate(BaseModel):
    """创建知识库任务请求"""
    source_type: str = Field(..., description="文件来源类型", pattern="^(file_upload|url)$")
...
class KBJobList(BaseModel):
    """知识库任务列表"""
    jobs: List[KBJobResponse] = Field(..., description="任务列表")
...
class KBDownloadResponse(BaseModel):
    """知识库下载响应"""
