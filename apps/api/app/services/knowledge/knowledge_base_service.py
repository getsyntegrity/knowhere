...
# ARQ依赖已移除，使用Celery替代
from app.services.common.global_manager_service import (
...
async def build_sim_matrix(user, source_node, topk=5, min_threshold=0.2):
    filter_path_vecs, _, filter_content_vecs, filter_contents_df = (
        checkerboard_filter_kb(
            user,
            signal_paths=["templates"],
            filter_mode="delete",
            target_types=["PTXT", "_TABLE", "_IMAGE"],
        )
    )
    masks = filter_contents_df["path"].str.contains(source_node, na=False, regex=False)
    self_ids = np.where(masks)[
        0
    ].tolist()  # 注意这里不能直接用filter_contents_df的index因为那些是整个 all_contents_df的对应位置

    top_content_ids, cthd = await gen_sim_matrix(
        filter_content_vecs, self_ids, topk, pre_threshold=min_threshold
    )
    top_path_ids, pthd = await gen_sim_matrix(
        filter_path_vecs, self_ids, topk, pre_threshold=min_threshold
    )
...
def filter_path(current_df, signal_paths: list, remain_paths: list, mode="delete"):
    if mode == "delete":
        filter_path_ids = [
            i
            for i, path in enumerate(remain_paths)
...
def checkerboard_filter_kb(user, signal_paths, target_types, filter_mode):
    ...
    # 然后进行路径过滤
    filter_paths, filter_path_ids, filter_content_df, filter_content_ids = filter_path(
        remain_df, signal_paths, remain_paths, mode=filter_mode
    )
...
async def checkerboard_find(
    ...
    path_vecs, all_paths, content_vecs, all_contents_df = checkerboard_filter_kb(
        user, signal_paths, target_types, filter_mode
    )
    ...
    sim_paths_pa, sim_ids_pa, scores_pa, _ = await find_closest(
        all_paths,
        path_vecs,
        q_vector,
        topk,
    ...
    _, sim_ids_con, scores_con, sim_paths_con = await find_closest(
        all_contents,
        content_vecs,
        q_vector,
        topk,
        msg=user_message,
        add_identifiers=all_paths,
        hybrid=hybrid,
        stopwords=user["stopwords"],
        token_corpus=all_contents_tokens,
        threshold=threshold,
    )
    ...
    merge_df = all_contents_df.iloc[sim_ids_merge][
    ...
    if rerank:
        ...
        rerank_df["merged_text"] = rerank_df["merged_text"].apply(
            lambda x: " ".join(x.split()[:200])
        )
        ...
        merged_paths = await rerank_(
            rerank_html, user_message, paths4rank, keep_one=True
        )
    else:
...
def matching_df(
    ...
    ini_filtered_df = all_contents_df[
        all_contents_df["path"].str.contains(sub_path, na=False, regex=False)
    ]
    ...
    for _, row in ini_filtered_df.iterrows():
        if "_SUMMARY" in row["type"]:
            ...
            summary_paths, summary_df = expand_summary_paths(
                all_contents_df, parent_node, summary_term
            )
    ...
    if save_session:
        exist_match_dfs = pd.read_csv(
            USER_SETTINGS["MATCH_DF"], encoding="utf-8", keep_default_na=False
        )
        all_match_dfs = pd.concat([exist_match_dfs, match_dfs], ignore_index=True)
        all_match_dfs.to_csv(
            USER_SETTINGS["MATCH_DF"],
            mode="a",
            index=False,
            encoding="utf-8",
            header=False,
        ...
        if "TABLE_" in row_type:
            ...
            for tb_id in tb_targets:
                if "TABLE_" in tb_id and not tb_id in used_tables:
                    ...
                    tb_str = read_tb_from_kb(
                        os.path.join(KB_PATH, tb_path), return_mode="html"
                    )

                    if len(tb_str) > long_table_limit:
                        logger.warning(
                            f"表格文本过长{len(tb_str)} 超过限制{long_table_limit} \t嵌入表格路径 后续将使用表格理解智能体..."
                        )
...
def checkerboard_integrate_contents(
    user, sim_paths, save_session=False, show_image=True, limit=200000
):
    all_contents_df = global_df_manager.get_dataframe(user["user"] + "_all_contents_df")
    ...
    while True and len(sim_paths) > 0:
        ...
        split_char = settings.SPLIT_CHAR or ";"
        show_path = split_char.join(
            sub_path.split(split_char)[user["USER_SETTINGS"]["ROOT_LEN"] :]
        )
        match_dfs, sub_summary = matching_df(
            sub_path, all_contents_df, user["USER_SETTINGS"], save_session=save_session
        )
        merged_df = merge_df(match_dfs)

        sub_content, used_images, used_tables = build_context(
            merged_df,
            resource_dict,
            used_images,
            used_tables,
...
async def talk2kb(query, context, paras):
    prompt, temperature, top_p, max_tokens = build_prompt(
        task="talk-kb", texts=context, query=query, paras=paras
    )
    messages = [
    ...
    ask_res = await ai_query_service.query_ai(
        messages=messages, user_id=ctx_task_id, conversation_id=ctx_task_id, timeout=60
    )
...
async def talk2kb_mm(query, context, paras):
    ...
    text_context = re.sub(
        r"(IMAGEURL_.*?_IMAGEURL|TABLEURL_.*?_TABLEURL)", "", context, flags=re.DOTALL
    )
    ...
    # 2. 基于图像（文本+图像）回答
    img_resp = await ask_image(
        client,
        paras["kb_path"],
        img_paths,
        title_text=text_context,
        task="ask-image",
        query=query,
        size_cut=False,
    )
    ...
    for tb_path in tb_paths:
        tb_context = await table_scope_analyze(
            query, os.path.join(paras["kb_path"], tb_path), paras
        )
    ...
    if len(mm_answers) > 0:
        ...
        ds_html = ds_df.style.set_table_styles(
            [
        ...
        prompt, temperature, top_p, max_tokens = build_prompt(
            task="merge-answers", texts=ds_html, query=task_query, paras=paras
        )
        messages = [
            {"role": "system", "content": "你是一个有帮助的助手"},
            {"role": "user", "content": prompt},
...
async def checkerboard_filling_tb(
    user, keyword, type, tb_texts, api_name="qwen_api", model_name="qwen-plus"
):
    if type == "KV":
        task = "filling-tb-kv"
    elif type == "CLICK":
        task = "filling-tb-ck"
    response, llm_histories = await use_llm_api(
...
def checkerboard_qlabel(user, query, qlabel, api_name="qwen_api"):
    fuzzy_warning = (
        "系统侦测到您的提问可能过于模糊，可能影响回答质量，请考虑是否优化提问。"
    )
    if qlabel:
        labeled_queries = []
        queries = [
            query
        ]  # UNDER DEVELOPMENT: divide a query into multiple ones if possible
        temp_labels = label_queries(
            queries,
            user["model_config"],
            user["USER_SETTINGS"],
            user.llm_histories,
        ...
        for lq in temp_labels:
            if lq[1] == "fuzzy":
                labeled_queries.append(
                    {"query": lq[0], "predefined_res": fuzzy_warning}
                )
            else:
    ...
    **kwargs,
    ...
    if user_config is None:
        ...
        if not user_config:
            ...
            user = (
                json.loads(user_dic_str)
                if isinstance(user_dic_str, str)
                else user_dic_str
            )
    ...
    base_llm_paras = {
        ...
        "summary_txt": kwargs.get("summary_txt", False),
    ...
    if ".txt" in file_full_path or ".fragment" in file_full_path:
        ...
        txt_lines = await parse_texts(
            file_path=file_full_path, fragment_content=fragment_content, baseurl=baseurl
        )
        await parse_md(
            kb_dir, source_type="md", md_lines=txt_lines, base_llm_paras=base_llm_paras
        )

    elif (
        ".png" in file_full_path
        or ".jpg" in file_full_path
        or ".jpeg" in file_full_path
    ) or ".fragment" in file_full_path:
        ...
        if filename is not None and file_full_path is not None:
            parsed_structure, df_list = await parse_docx(
                file_full_path, base_llm_paras, kb_dir, filename, baseurl
            )
            await convert_doc2dics(
                parsed_structure, df_list, kb_dir, base_llm_paras=base_llm_paras
            )
        ...
        if filename is not None and file_full_path is not None:
            await parse_xlsx(
                file_full_path, filename, kb_dir, baseurl, base_llm_paras=base_llm_paras
            )
...
def checkerboard_learn(
    user, reply, user_intention, sim_contents, user_selected_ids, current_markers
):
    # 改造为微调大模型
    local_train_data = gen_train_data_from_interactions(
        user["USER_SETTINGS"],
        user_intention,
        sim_contents,
        user_selected_ids,
    ...
    if user["USER_SETTINGS"]["BN_RL"] and (
        len(local_train_data) >= user["USER_SETTINGS"]["N_TRIGGER"]
        and len(local_train_data) % user["USER_SETTINGS"]["N_TRIGGER"] == 0
    ):
        ...
        user.opt_memory.eval(
            len(reply), user_intention, sim_contents, user_selected_ids, current_markers
        )
...
def encapsulate_json(df, kb_dir):
    ...
    for _, row in df.iterrows():
        chunk = {
            "chunk_id": str(
                uuid.uuid5(uuid.NAMESPACE_DNS, str(row.get("know_id", uuid.uuid4())))
            ),
        ...
        if row.get("type").startswith("PTXT"):
            ...
            chunk.get("metadata").update(
                {"file_path": f"images/{img_name}", "original_name": img_name}
            )
            ...
            chunk.get("metadata").update(
                {"file_path": f"images/{tbl_name}", "original_name": tbl_name}
            )
        chunks.append(chunk)
