...
async def buy_credits_for_user(
    *,
...
async def get_credits_balance_for_user(
    db: AsyncSession,
    *,
    user_id: str,
) -> CreditsBalanceResponse:
...
async def get_parse_usage_overview_for_user(
    db: AsyncSession,
    *,
    user_id: str,
) -> ParseUsageResponse:
    try:
        total_micro_credits_used = await _load_total_parse_micro_credits_used(
            db,
            user_id=user_id,
        )
        success_rate, avg_processing_time = await _load_parse_job_usage_stats(
            db,
            user_id=user_id,
        )
        estimated_amount = await _estimate_parse_usage_amount(
            db,
...
async def get_price_configs_payload(
    db: AsyncSession,
    *,
...
async def buy_credits_package_for_user(
    db: AsyncSession,
    *,
    ...
    try:
        ...
        raise StripeServiceException(
            internal_message=f"Failed to create credits package purchase: {str(exc)}"
        )
...
async def _load_parse_job_usage_stats(
    ...
    *,
    ...
    success_rate = (done_count / terminal_count * 100) if terminal_count > 0 else 0.0
...
def _subscription_config_payload(config) -> dict:
...
def _credits_package_config_payload(config) -> dict:
    metadata = config.extra_metadata or {}
