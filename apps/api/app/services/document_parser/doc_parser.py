...
async def handle_image(
    ...
    try:
        ...
        last_context = re.sub(
            re.compile(r"(TABLE_.*?_TABLE|IMAGE_.*?_IMAGE)"), "", last_context
        )
    except:
        last_context = ""
    ...
    raw_img_name = process_path_texts(
        f"图-{current_heading} {last_context}", last=30
    ) + str(img_count)
    ...
    img_path = settings.SPLIT_CHAR.join(
        img_dir.split(os.sep) + [f"{img_name}{img_ext}"]
    )
...
async def handle_table(
    ...
    raw_tb_name = (
        " ".join([cell.text.strip() for cell in block.rows[0].cells])
        if block.rows
        else f"表格{table_count}表头"
    )
    ...
    tb_name = process_path_texts(f"表-{raw_tb_name} {tb_summary}", last=30) + str(
        table_count
    )
...
async def parse_docx(
    ...
    block_tuples = [
        b for b in block_tuples if not "TOC" in b[2]
    ]  # TODO temporary remove toc area
    logger.debug(f"解析文档元素数量: {len(block_tuples)}")
    heading_infos = []
    ...
    if not llm_paras["doc_type"] in "templates":
        heading_candidates = await pred_titles(
            heading_infos,
            doc_type="docx",
            enable_regx=True,
    ...
    for block_tuple in tqdm(
        block_tuples, total=len(block_tuples), desc="Parsing docx file..."
    ):
...
async def convert_doc2dics(parsed_structure, df_list, kb_dir, base_llm_paras):
    ...
    for _, row in leaf_dics.iterrows():
        ...
        try:
            ...
            know_path = split_char.join(
                kb_dir.split(os.sep) + ([key] if key.strip() else [])
            )
            df_list.append(
                [
                    bottom_content,
