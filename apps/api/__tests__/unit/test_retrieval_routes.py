...
@pytest.mark.asyncio
async def test_retrieval_query_returns_canonical_chunk_results(
    authenticated_client, mock_db
):
    from app.api.v1.routes import retrieval as retrieval_routes
...
@pytest.mark.asyncio
async def test_document_routes_return_canonical_document_state(
...
@pytest.mark.asyncio
async def test_document_list_defaults_namespace_when_omitted(
    authenticated_client, monkeypatch
):
    from app.api.v1.routes import documents as document_routes
...
@pytest.mark.asyncio
async def test_document_archive_route_supports_slash_action_alias(
    authenticated_client, monkeypatch
):
    from app.api.v1.routes import documents as document_routes
...
@pytest.mark.asyncio
async def test_document_get_route_returns_404_when_document_is_missing(
    authenticated_client, monkeypatch
):
    from app.api.v1.routes import documents as document_routes
...
@pytest.mark.asyncio
async def test_document_archive_route_returns_404_when_document_is_missing(
    authenticated_client, monkeypatch
):
    from app.api.v1.routes import documents as document_routes
...
@pytest.mark.asyncio
async def test_retrieval_query_schedules_usage_analytics_best_effort(
    authenticated_client, monkeypatch
):
    from app.api.v1.routes import retrieval as retrieval_routes
    ...
    response = await authenticated_client.post(
        "/v1/retrieval/query", json={"query": "refund policy", "top_k": 5}
    )
...
@pytest.mark.asyncio
async def test_retrieval_query_ignores_usage_analytics_schedule_failure(
    authenticated_client, monkeypatch
):
    from app.api.v1.routes import retrieval as retrieval_routes
...
@pytest.mark.asyncio
async def test_retrieval_query_route_uses_shared_app_service(
    authenticated_client, monkeypatch
):
    from app.api.v1.routes import retrieval as retrieval_routes
...
@pytest.mark.asyncio
async def test_retrieval_query_route_returns_cached_result_from_shared_service(
    authenticated_client, monkeypatch
):
    from app.api.v1.routes import retrieval as retrieval_routes
...
@pytest.mark.asyncio
async def test_retrieval_query_route_passes_section_exclusions(
    authenticated_client, monkeypatch
):
    from app.api.v1.routes import retrieval as retrieval_routes
...
async def test_retrieval_asset_url_helper_works_in_api_runtime(monkeypatch):
    ...
    monkeypatch.setattr(
        "shared.services.retrieval.app_service.get_result_storage",
        lambda: FakeResultStorage(),
    )

    assert await generate_retrieval_asset_url(
        job_id="job_123", artifact_ref="images/page-1.png"
    ) == ("https://assets.test/results/job_123/images/page-1.png?signature=fresh")
    assert captured == {
        "job_id": "job_123",
...
@pytest.mark.asyncio
async def test_archive_canonical_document_invalidates_namespace_cache_best_effort(
    monkeypatch,
):
    ...
    monkeypatch.setattr(
        "app.services.document_service.DocumentGraphService", FakeGraphService
    )
    monkeypatch.setattr(
        "app.services.document_service.invalidate_retrieval_cache_namespaces",
        fake_invalidate_retrieval_cache_namespaces,
    )
    ...
    class FakeDb:
        ...
        async def run_sync(self, _fn):
            raise AssertionError(
                "graph removal should not run for an already archived document"
            )

        async def commit(self):
            raise AssertionError(
                "commit should not run for an already archived document"
            )
...
def test_document_routes_keep_db_logic_out_of_router():
    source = (Path(__file__).parents[2] / "app/api/v1/routes/documents.py").read_text(
        encoding="utf-8"
    )
