...
async def test_real_mcp_runtime_registers_and_calls_kb_query(monkeypatch):
    ...
    monkeypatch.setattr(
        retrieval_server, "run_retrieval_query", fake_run_retrieval_query
    )
    monkeypatch.setattr(
        retrieval_server, "resolve_mcp_user_id", fake_resolve_mcp_user_id
    )
    ...
    assert (
        ...
        == "Search for information and return relevant knowledge snippets."
    )
...
async def test_create_retrieval_mcp_server_registers_kb_query_tool(monkeypatch):
    ...
    monkeypatch.setattr(
        retrieval_server, "get_current_user_id", fake_get_current_user_id
    )
...
@pytest.mark.asyncio
async def test_kb_query_requires_db_factory_to_return_async_context_manager(
    monkeypatch,
):
    from app.mcp import retrieval_server
