...
def upgrade() -> None:
    ...
    sa.Column('id', sa.String(length=36), nullable=False, comment='Directory record identifier'),
    sa.Column('title', sa.String(length=255), nullable=False, comment='Directory title'),
    sa.Column('parent_id', sa.String(length=36), nullable=True, comment='Parent directory ID; NULL means root directory'),
    sa.Column('user_id', sa.String(length=36), nullable=False, comment='Owning user ID'),
    ...
    sa.ForeignKeyConstraint(['parent_id'], ['file_directory.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_file_directory_id'), 'file_directory', ['id'], unique=False)
    op.create_table('knowledge_base',
    sa.Column('id', sa.String(length=36), nullable=False, comment='Content row identifier'),
    ...
    sa.Column('type', sa.String(length=2000), nullable=True, comment='Content type, for example PTXT or SUMMARY'),
    sa.Column('length', sa.Integer(), nullable=True, comment='Content length or size'),
    sa.Column('keywords', sa.String(length=511), nullable=True, comment='Content keywords'),
    sa.Column('summary', sa.Text(), nullable=True, comment='Content summary'),
    sa.Column('know_id', sa.String(length=128), nullable=True, comment='Knowledge record ID that may refer to an external knowledge base'),
    sa.Column('tokens', sa.Text(), nullable=True, comment='Tokenized content'),
    sa.Column('embedding', sa.Text(), nullable=True, comment='Semantic vector used for similarity search'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_knowledge_base_id'), 'knowledge_base', ['id'], unique=False)
    op.create_table('path_base',
    sa.Column('id', sa.String(length=36), nullable=False, comment='Path row identifier'),
    sa.Column('path', sa.Text(), nullable=True, comment='File path or source'),
    sa.Column('embedding', sa.Text(), nullable=True, comment='Semantic vector used for similarity search'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_path_base_id'), 'path_base', ['id'], unique=False)
    op.create_table('stripe_price_configs',
    ...
    sa.Column('credits_amount', sa.BigInteger(), server_default='0', nullable=False),
    ...
    sa.CheckConstraint("(product_type = 'credits_package' AND credits_amount > 0) OR (product_type = 'subscription')", name='chk_credits_package_has_amount'),
    ...
    op.create_index(op.f('ix_stripe_price_configs_price_id'), 'stripe_price_configs', ['price_id'], unique=True)
    ...
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    sa.Column('page_count', sa.Integer(), nullable=True, comment='Calculated page count for billing'),
    sa.Column('credits_charged', sa.BigInteger(), nullable=False, comment='In micro-dollars: $1.00 = 1,000,000'),
    sa.Column('billing_status', sa.String(length=50), nullable=False, comment='pending, charged, billing_failed, refunded'),
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], ondelete='RESTRICT'),
    ...
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], ondelete='RESTRICT'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('checkout_session_id', name='uq_payment_record_checkout_session_id')
    )
    op.create_index(op.f('ix_payment_records_checkout_session_id'), 'payment_records', ['checkout_session_id'], unique=True)
    op.create_index(op.f('ix_payment_records_payment_intent_id'), 'payment_records', ['payment_intent_id'], unique=True)
    op.create_index(op.f('ix_payment_records_stripe_subscription_id'), 'payment_records', ['stripe_subscription_id'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_payment_records_stripe_subscription_id'), table_name='payment_records')
    op.drop_index(op.f('ix_payment_records_payment_intent_id'), table_name='payment_records')
    op.drop_index(op.f('ix_payment_records_checkout_session_id'), table_name='payment_records')
    ...
    op.drop_index(op.f('ix_credits_transactions_stripe_payment_id'), table_name='credits_transactions')
    ...
    op.drop_index(op.f('ix_stripe_price_configs_price_id'), table_name='stripe_price_configs')
