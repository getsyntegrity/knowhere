...
def test_should_prefer_mineru_s3_url_mode_by_default_even_in_development(
    ...
    monkeypatch.delenv("FORCE_MINERU_UPLOAD_ENABLED", raising=False)
...
def test_should_honor_legacy_force_upload_flag_when_new_flag_is_unset(
    worker_contract_environment: None,
    monkeypatch: MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    monkeypatch.setenv("ENVIRONMENT", "production")
    monkeypatch.delenv("MINERU_UPLOAD_MODE_ENABLED", raising=False)
    monkeypatch.setenv("FORCE_MINERU_UPLOAD_ENABLED", "true")
    pdf_service = _load_mineru_pdf_service()
    ...
    def fake_request_upload_target(
        ...
        assert pdf_url.endswith("source.pdf")
        assert filename == "contract.pdf"
        return "batch_legacy_direct", "https://mineru-upload.example.com/file", "token"
...
def test_should_prefer_new_upload_mode_flag_over_legacy_force_flag(
    worker_contract_environment: None,
    monkeypatch: MonkeyPatch,
    tmp_path: Path,
) -> None:
    del worker_contract_environment
    monkeypatch.setenv("ENVIRONMENT", "production")
    monkeypatch.setenv("MINERU_UPLOAD_MODE_ENABLED", "false")
    monkeypatch.setenv("FORCE_MINERU_UPLOAD_ENABLED", "true")
    pdf_service = _load_mineru_pdf_service()
    ...
    def fake_submit_url_task(presigned_url: str, filename: str) -> tuple[str, str]:
        submit_calls.append(f"{filename}:{presigned_url}")
        return "batch_new_flag", "token"

    def fail_direct_upload(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("legacy direct-upload flag must not override new flag")
    ...
    assert storage_calls == ["uploads/contract.pdf"]
    assert submit_calls == [
        "contract.pdf:https://files.example.com/uploads/contract.pdf"
    ]
