...
def _fake_db_context(job_status: str):
    result = MagicMock()
    result.scalar_one_or_none.return_value = SimpleNamespace(job_id="job_123", status=job_status)
...
def test_mark_job_running_transitions_pending_job(monkeypatch):
    monkeypatch.setattr(job_start_service, "get_sync_db_context", lambda: _fake_db_context(JobStatus.PENDING.value))
...
def test_mark_job_running_retries_when_job_still_waiting_for_file(monkeypatch):
    monkeypatch.setattr(job_start_service, "get_sync_db_context", lambda: _fake_db_context(JobStatus.WAITING_FILE.value))
...
def test_mark_job_running_is_idempotent_for_running_job(monkeypatch):
    monkeypatch.setattr(job_start_service, "get_sync_db_context", lambda: _fake_db_context(JobStatus.RUNNING.value))
