...
| --- | --- | --- | --- |
| `api_key` | `/api/v1/auth/*` | `Partial` | `create`, `list`, `revoke`, and revoked-key rejection are covered. |
| `guest` | `/api/v1/guest` | `Full` | Guest registration and duplicate-device conflict are covered. |
| `billing` | `/api/v1/billing/*` | `None` | No API contract coverage yet. |
| `knowledge_base` | `/api/v1/kb/*` | `None` | No API contract coverage yet. |
| `jobs` | `/api/v1/jobs/*` | `Full` | File mode, URL mode, reads, confirm-upload, and all live jobs `429` layers are covered. |
| `retrieval` | `/api/v1/retrieval/query` | `None` | No API contract coverage yet. |
| `documents` | `/api/v1/documents/*` | `None` | No API contract coverage yet. |
| `s3_events` | `/api/v1/internal/s3-events` | `None` | No API contract coverage yet. |
| `webhook` | `/api/v1/webhooks/*` | `None` | No API contract coverage yet. |
| `webhook_secrets` | `/api/v1/webhooks/secrets/*` | `None` | No API contract coverage yet. |
| `qstash_callbacks` | `/api/v1/webhooks/qstash/*` | `None` | No API contract coverage yet. |
| `health` | `/api/v1/health/database/*` | `None` | The suite covers app-root `/api/health`, not these v1 database-health endpoints. |
| `version` | `/api/v1/version`, `/api/v1/` | `None` | No API contract coverage yet. |
