# 阿里云 ECS Docker Compose 部署文档

> **重要说明**：本文档仅适用于 **test 环境（staging 分支）** 的部署。
> 
> - **Test 环境**：使用本方案（ECS + Docker Compose）
> - **Prod 环境**：使用 ACK (Kubernetes) Serverless 方案，请参考 [阿里云部署指南](../DEPLOYMENT_ALIYUN.md)

本文档说明如何在阿里云 ECS 服务器上使用 Docker Compose 部署 Knowhere 应用的 test 环境。
...
- 域名已解析到 ECS 服务器 IP
- API 域名: `api.knowhereto.com` 或 `apitest.knowhereto.com`
- Web 域名: `knowhereto.com` 或 `test.knowhereto.com`

### 本地要求

- 本地已配置 SSH 访问 ECS 服务器
- 已准备环境变量文件 `.env`
...
```bash
cp deploy/aliyun/.env.staging.template .env
```
...
- `CELERY_BROKER_URL`: 使用 `amqp://admin:admin123@rabbitmq:5672//`（不是 `localhost`）
...
```bash
scp .env ${ECS_USER}@${ECS_HOST}:/var/lib/knowhere/.env
ssh ${ECS_USER}@${ECS_HOST} "chmod 600 /var/lib/knowhere/.env"
```
...
# 设置域名（如需要）
export SSL_DOMAINS="api.knowhereto.com knowhereto.com"
export SSL_EMAIL="admin@knowhereto.com"
...
# 查看特定服务日志
docker-compose -f docker-compose.ecs.yml logs -f api
docker-compose -f docker-compose.ecs.yml logs -f web
docker-compose -f docker-compose.ecs.yml logs -f worker
```
...
# 停止并删除数据卷（谨慎使用）
docker-compose -f docker-compose.ecs.yml down -v
```
...
```bash
# 拉取最新镜像并重启
docker-compose -f docker-compose.ecs.yml pull
...
1. 检查服务日志：
   ```bash
   docker-compose -f docker-compose.ecs.yml logs [服务名]
   ```
...
```
/var/lib/knowhere/
├── docker-compose.ecs.yml    # Docker Compose 配置文件
├── .env                       # 环境变量文件
├── data/                      # 数据目录
...
├── logs/                      # 应用日志
├── ssl/                       # SSL 证书（已弃用，使用 /etc/letsencrypt）
├── nginx/                     # Nginx 配置
│   └── nginx.conf
└── scripts/                   # 部署脚本
    ├── deploy-to-ecs.sh
...
2. **部署配置文件**: 确保 `deploy-config.sh` 文件权限为 600（如果使用）
   ```bash
   chmod 600 deploy/aliyun/deploy-config.sh
   ```
...
A: 编辑 `docker-compose.ecs.yml` 文件中的镜像标签，然后重新部署。
...
- API: `https://api.knowhereto.com/health`
- Web: `https://knowhereto.com/health`
