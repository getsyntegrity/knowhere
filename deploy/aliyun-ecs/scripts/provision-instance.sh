...
# 8. 配置systemd服务
log "配置systemd服务..."
cp "$APP_DIR/deploy/aliyun-ecs/systemd/knowhere-api.service" /etc/systemd/system/
cp "$APP_DIR/deploy/aliyun-ecs/systemd/knowhere-web.service" /etc/systemd/system/
cp "$APP_DIR/deploy/aliyun-ecs/systemd/knowhere-worker.service" /etc/systemd/system/
...
# 9. 配置Nginx
log "配置Nginx..."
cp "$APP_DIR/deploy/aliyun-ecs/nginx/knowhere.conf" /etc/nginx/sites-available/
cp "$APP_DIR/deploy/aliyun-ecs/nginx/nginx.conf" /etc/nginx/nginx.conf || true
cp "$APP_DIR/deploy/aliyun-ecs/nginx/ssl-params.conf" /etc/nginx/ssl-params.conf || true
...
if [ ! -f "$APP_DIR/.env" ]; then
    if [ -f "$APP_DIR/deploy/aliyun-ecs/config/env.template" ]; then
        cp "$APP_DIR/deploy/aliyun-ecs/config/env.template" "$APP_DIR/.env"
    fi
...
case "$1" in
    ...
    "app")
        ...
        ;;
    *)
        echo "用法: $0 {api|web|worker|nginx|app}"
        echo "  api     - 查看API服务日志"
        echo "  web     - 查看Web服务日志"
