    ...
    "logs": {
        "logs_collected": {
            "files": {
                "collect_list": [
                    {
                        "file_path": "/opt/knowhere/logs/*.log",
                        "log_group_name": "/aws/ec2/knowhere-test",
                    ...
                    },
                    {
                        ...
                        "log_group_name": "/aws/ec2/knowhere-test",
                        "log_stream_name": "{instance_id}/nginx-logs",
                        "timezone": "UTC"
                    }
                ]
            }
...
# S3配置
S3_BUCKET_NAME=knowhere-test-storage
S3_ACCESS_KEY_ID=
...
# 创建部署脚本目录
mkdir -p /opt/knowhere/deploy/aws-ec2/scripts
chown -R appuser:appuser /opt/knowhere/deploy

# 复制SSH密钥到实例
log "复制SSH密钥..."
if [ -f "/tmp/repo-git" ]; then
    cp /tmp/repo-git /opt/knowhere/deploy/aws-ec2/scripts/
    chmod 600 /opt/knowhere/deploy/aws-ec2/scripts/repo-git
    chown appuser:appuser /opt/knowhere/deploy/aws-ec2/scripts/repo-git
    log "SSH密钥已复制"
else
...
log "EC2实例初始化完成！"
log "请运行以下命令完成应用部署："
log "1. cd /opt/knowhere/deploy/aws-ec2/scripts"
