...
check_aws_credentials() {
    ...
    log "AWS账户ID: $AWS_ACCOUNT_ID"
    log "AWS区域: $AWS_REGION"
}
...
wait_for_instance() {
    ...
    while [ $attempt -le $max_attempts ]; do
        if ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no ubuntu@$INSTANCE_IP "echo 'Instance is ready'" &> /dev/null; then
            log "实例已就绪"
            return 0
        fi
...
deploy_application() {
    ...
    # 复制部署脚本到实例
    log "复制部署脚本到实例..."
    scp -o StrictHostKeyChecking=no -r "$SCRIPT_DIR" ubuntu@$INSTANCE_IP:/tmp/
    
    # 在实例上运行配置脚本
    log "在实例上运行配置脚本..."
    ssh -o StrictHostKeyChecking=no ubuntu@$INSTANCE_IP "
        sudo cp -r /tmp/scripts /opt/knowhere/deploy/aws-ec2/
...
show_deployment_info() {
    ...
    log ""
    log "SSH连接命令："
    log "  ssh -i ~/.ssh/your-key.pem ubuntu@$INSTANCE_IP"
    log ""
    log "服务管理命令："
