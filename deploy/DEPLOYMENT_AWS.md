...
> **重要说明**：不同环境采用不同的部署方案
> 
> - **Test 环境**（staging 分支）：使用 **EC2 + Docker Compose** 方案（见 [Test 环境部署](#test环境部署)）
> - **Prod 环境**（main 分支）：使用 **ECS Fargate Serverless** 方案（见 [Prod 环境部署](#prod环境部署)）
...
Test 环境使用 **EC2 + Docker Compose** 方案：
...
Prod 环境使用 **ECS Fargate Serverless** 方案：
...
### Test 环境部署（EC2 + Docker Compose）

Test 环境使用固定 EC2 服务器和 Docker Compose 部署。
...
4. **部署服务**
   ...
   export GITHUB_USERNAME=your-github-username
   ./deploy/aws/scripts/deploy-to-ec2.sh
   ```

5. **配置 DNS**
   - 将 `apitest.knowhereto.com` 和 `test.knowhereto.com` 解析到 EC2 公网 IP

**详细说明**：Test 环境的完整部署流程请参考部署脚本 `deploy/aws/scripts/deploy-to-ec2.sh`

### Prod 环境部署（ECS Fargate Serverless）

Prod 环境使用 ECS Fargate 和 Serverless 基础设施。
...
1. **自动触发**: 推送到 `staging` 或 `main` 分支时自动触发构建
2. **构建镜像**: GitHub Actions 构建三个镜像（backend、frontend、worker）
3. **推送镜像**: 自动推送到 GHCR，标签为 `staging-latest` 或 `main-latest`

### 镜像标签策略

- `staging-latest`: test 环境最新版本
- `main-latest`: prod 环境最新版本
- `v*`: 版本标签（如 `v1.0.0`）

### 版本号获取

GitHub Actions 会自动从 Git 获取版本号：
- 如果有精确匹配的 Tag → 使用 Tag（如 `v1.0.0`）
- 如果有 Tag 但不是精确匹配 → 使用 Tag+分支名称（如 `v1.0.0-staging`）
- 如果没有 Tag → 使用 `分支名称-latest`（如 `staging-latest`）

详细说明请参考：[GitHub Actions 构建指南](GITHUB_ACTIONS_BUILD.md)

## 基础设施部署

### Terraform 配置（仅用于 Prod 环境）

**重要**：以下步骤仅适用于 **prod 环境**（ECS Fargate Serverless）。test 环境使用 EC2 + Docker Compose，不需要 Terraform 配置。
...
编辑 `terraform.tfvars.prod`，设置以下变量：

```hcl
aws_region = "us-east-1"
project_name = "knowhere"
environment = "prod"  # 必须设置为 "prod"
...
- **test 环境**（staging 分支）：使用 **EC2 + Docker Compose** 方案
  - 使用固定 EC2 服务器
  - 所有服务通过 Docker Compose 管理
  - 基础服务（数据库、缓存、消息队列）运行在容器中
- **prod 环境**（main 分支）：使用 **ECS Fargate Serverless** 方案
  - 使用 ECS Fargate 无服务器容器
  - 所有基础设施 Serverless
  - 本文档主要说明此方案
...
Test 环境使用 EC2 + Docker Compose，配置方式不同：

- 使用 Docker Compose 配置文件（如存在：`deploy/aws/docker-compose.ec2.yml`）
- 环境变量文件：`.env`（需要手动创建）
...
**重要**：Terraform 仅用于 **prod 环境**（ECS Fargate Serverless）部署。

以下配置仅适用于 **prod 环境**：

- 配置文件：`terraform.tfvars.prod`
- Terraform State：S3 Backend（`knowhere-terraform-state-prod`）
- 资源：ECS 集群、RDS、S3、ALB 等（通过 Terraform 管理）
- 域名和证书：自动配置
...
| ECS 服务实例数 | 2 |
| RDS 实例数 | 2 (高可用) |
| RabbitMQ 模式 | 多 AZ 高可用 |
| 日志保留 | 30 天 |
| 删除保护 | 开启 |

详细 Terraform 配置请参考：[AWS Terraform 配置指南](aws/terraform/README.md)
